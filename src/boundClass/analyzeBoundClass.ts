...
const analyzeJiNotationBoundClass = (
    ...
    const possibleBoundHistories = boundHistoryAnalyses.filter(
        (boundHistoryAnalysis: BoundHistoryAnalysis): boolean => boundHistoryAnalysis.possible,
    )
    ...
    if (isUndefined(bestPossibleBoundHistoryAnalysis)) {
        throw new Error(
            `Unable to find a best possible bound class history for bound class ${stringify(jiNotationBoundClass)}`,
        )
    }
    ...
    const initialPositionTinaDistance = (subtractPitch(jiNotationBoundClass.pitch, initialPosition) /
        TINA_CENTS) as Multiplier<Tinas>

    updateRankAnalysis(bestRank, boundClassId)
    updateJiNotationLevelAnalysis(bestPossibleBoundHistoryAnalysis)

    const boundHistoryConsolidation = consolidateBoundHistories(
    ...
    } as unknown as JiNotationBoundClassAnalysis
}

export { analyzeJiNotationBoundClass }
