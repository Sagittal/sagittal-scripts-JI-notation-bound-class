...
const computeExtendedJiNotationLevelBoundHistories = (
    ...
    let extendedJiNotationLevelBoundHistories: BoundHistory[] = computeExtensionBase(
        ExtensionBaseType.ARRAY,
    ) as BoundHistory[]

    boundHistories.forEach((boundHistory: BoundHistory): void => {
        const extendedBoundHistories = computeExtendedHistories(
            boundHistory,
            jiNotationLevel,
            jiNotationBoundClass,
        )
        extendedJiNotationLevelBoundHistories =
            extendedJiNotationLevelBoundHistories.concat(extendedBoundHistories)
    })

    return extendedJiNotationLevelBoundHistories
}

export { computeExtendedJiNotationLevelBoundHistories }
