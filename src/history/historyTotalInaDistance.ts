...
const computeBoundHistoryTotalInaDistance = (
    ...
    const inaDistances = boundEventAnalyses.map(
        (boundEventAnalysis: BoundEventAnalysis): Multiplier<Ina> => boundEventAnalysis.inaDistance,
    )

    return sum(...inaDistances)
}

export { computeBoundHistoryTotalInaDistance }
