...
const visualizeBoundEventAnalyses = (boundEventAnalyses: BoundEventAnalysis[]): Io[] => {
    ...
    const {
        pitch: initialPosition,
        rank: initialRank,
        jiNotationLevel: initialLevel,
    } = initialBoundEventAnalysis
    ...
    boundEventAnalyses.forEach(({ jiNotationLevel, pitch }: BoundEventAnalysis, index: number): void => {
        ...
        boundEventElements.push(
            `  <line stroke="${stroke}" x1="${positionX}" y1="${positionY}" x2="${nextPositionX}" y2="${nextPositionY}" />\n` as Io,
        )
        boundEventElements.push(
            `  <circle stroke="${stroke}" r="${DOT_SIZE}" cx="${nextPositionX}" cy="${nextPositionY}" />\n` as Io,
        )
        ...
        boundEventElements.push(
            `  <text stroke="white" stroke-width="0.45em" transform="rotate(${angle} ${textX} ${textY})" text-anchor="middle" alignment-baseline="hanging" xml:space="preserve" font-family="Helvetica" font-size="6px" x="${textX}" y="${textY}">${formattedDistance} (${formattedInaDistance})</text>\n` as Io,
        )
        boundEventElements.push(
            `  <text fill="red" transform="rotate(${angle} ${textX} ${textY})" text-anchor="middle" alignment-baseline="hanging" xml:space="preserve" font-family="Helvetica" font-size="6px" x="${textX}" y="${textY}">${formattedDistance} (${formattedInaDistance})</text>\n` as Io,
        )
    })
...
export { visualizeBoundEventAnalyses }
