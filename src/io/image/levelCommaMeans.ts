...
import { computeAccidentalUnicode, JiNotationLevelId, parseSagitype, Sagitype } from "@sagittal/system"
    ...
    jiNotationLevelCommaMeansEntries.forEach(
        ([jiNotationLevel, jiNotationLevelCommaMeans]: [JiNotationLevelId, CommaMean[]]): void => {
            ...
            jiNotationLevelCommaMeans.forEach(({ name, pitch }: CommaMean): void => {
                ...
                jiNotationLevelCommaMeanElements.push(
                    `  <line stroke-dasharray="${DASH_SIZE}" stroke="${COMMA_MEAN_HEX_COLOR}" x1="${positionX}" x2="${positionX}" y1="${topY}" y2="${bottomY}"/>\n` as Io,
                )
                jiNotationLevelCommaMeanElements.push(
                    `  <text fill="white" alignment-baseline="hanging" text-anchor="middle" xml:space="preserve" x="${positionX}" y="${bottomY}" font-size="6px" font-family="Helvetica">${name}</text>\n` as Io,
                ) // So they can be searched by Sagitype
                jiNotationLevelCommaMeanElements.push(
                    `  <text stroke="white" stroke-width="0.45em" alignment-baseline="hanging" text-anchor="middle" xml:space="preserve" x="${positionX}" y="${bottomY}" font-size="10px" font-family="Bravura">${formattedName}</text>\n` as Io,
                )
                jiNotationLevelCommaMeanElements.push(
                    `  <text fill="${COMMA_MEAN_HEX_COLOR}" alignment-baseline="hanging" text-anchor="middle" xml:space="preserve" x="${positionX}" y="${bottomY}" font-size="10px" font-family="Bravura">${formattedName}</text>\n` as Io,
                )
            })
        },
    )
...
export { visualizeJiNotationLevelCommaMeans }
